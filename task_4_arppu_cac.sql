...
with p1 as (SELECT 'Кампания № 1' as ads_campaign, user_id, time
FROM user_actions
...
10105, 10122, 10134, 10135)
UNION
SELECT 'Кампания № 2' as ads_campaign, user_id, time
FROM user_actions
...
paying_users AS (
  SELECT DISTINCT user_id, ads_campaign,
    MIN(time::date) OVER(PARTITION BY user_id, ads_campaign) AS start_date
  FROM p1
  WHERE user_id IN (
...
rev_tab AS (
  SELECT 
    ads_campaign, 
    date,
    SUM(summa) OVER(PARTITION BY ads_campaign ORDER BY date ROWS UNBOUNDED PRECEDING) AS revenue
  FROM (
      SELECT
        pu.ads_campaign,
      ...
      FROM paying_users as pu
        JOIN user_actions ua ON pu.user_id = ua.user_id 
            AND ua.action = 'create_order'
            AND ua.order_id NOT IN (SELECT order_id FROM user_actions WHERE action = 'cancel_order')
        JOIN orders o ON ua.order_id = o.order_id
        JOIN (
            SELECT order_id, unnest(product_ids) AS product_id_a
            FROM orders
            WHERE order_id NOT IN (SELECT order_id FROM user_actions WHERE action = 'cancel_order')
        ) unnested ON o.order_id = unnested.order_id
        JOIN products p ON p.product_id = unnested.product_id_a
      GROUP BY pu.ads_campaign, date ) rev_prep
)
SELECT
  rt.ads_campaign,
  CONCAT('Day ', (rt.date - cc.cohort_start)::text) AS day,
  ROUND(rt.revenue::decimal / cc.cuser, 2) AS cumulative_arppu,
  cc.cac
FROM rev_tab rt
JOIN campaign_cac cc ON rt.ads_campaign = cc.ads_campaign
ORDER BY rt.ads_campaign, (rt.date - cc.cohort_start)
