...
SELECT ads_campaign, start_date, time-start_date as day_number,
    round(COUNT(DISTINCT user_id)::decimal / (
        SELECT COUNT(DISTINCT user_id) 
        FROM (
             SELECT DISTINCT user_id, MIN(time::date) OVER(PARTITION BY user_id) AS cohort_start
             FROM p1 
             WHERE ads_campaign = t1.ads_campaign) cohort 
        WHERE cohort_start = t1.start_date 
     ), 2) as retention
FROM (
        SELECT distinct user_id, time::date, ads_campaign,
        min(time::date) OVER(PARTITION BY user_id) as start_date
        from p1 
     ) as t1
WHERE time-start_date in (0 , 1, 7)
GROUP BY ads_campaign, time, start_date
ORDER BY ads_campaign, start_date, time
